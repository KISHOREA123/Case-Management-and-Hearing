...
const CasesPage = () => {
    ...
    const [formData, setFormData] = useState({
        case_title: '',
        case_number: '',    // read-only, auto-generated by server
        case_type_id: '',
        court_id: '',
        petitioner: '',
        defender: '',
    ...
    const handleEdit = (c) => {
        ...
        setFormData({
            ...
            case_type_id: c.case_type_id?._id || c.case_type_id,
            court_id: c.court_id?._id || c.court_id,
            petitioner: c.petitioner,
            defender: c.defender,
            filing_date: c.filing_date ? new Date(c.filing_date).toISOString().split('T')[0] : '',
            description: c.description || '',
            priority: c.priority || 'Medium',
            lawyer_id: c.lawyer_id?._id || c.lawyer_id || (user?.role === 'admin' ? '' : user?._id)
        });
        setIsModalOpen(true);
    };
            ...
            <div className="bg-card border border-border rounded-xl">
                <div className="p-4 border-b border-border flex flex-col sm:flex-row gap-4">
                    <div className="relative flex-1">
                        ...
                        <input
                            type="text"
                            placeholder={user?.role === 'admin' ? "Search by title or number..." : "Search by title, number, or petitioner..."}
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-accent/50 text-muted-foreground text-xs uppercase tracking-wider">
                                ...
                                {user?.role !== 'admin' && <th className="px-6 py-4 font-semibold">Petitioner</th>}
                        ...
                        <tbody className="divide-y divide-border">
                            {loading ? (
                                <tr><td colSpan={user?.role === 'admin' ? "6" : "7"} className="px-6 py-8 text-center text-muted-foreground">Loading cases...</td></tr>
                            ) : paginatedCases.length === 0 ? (
                                <tr><td colSpan={user?.role === 'admin' ? "6" : "7"} className="px-6 py-8 text-center text-muted-foreground">No cases found.</td></tr>
                                ...
                                <tr key={c._id} className="hover:bg-accent/30 transition-colors text-sm">
                                    ...
                                    <td className="px-6 py-4">{c.case_type_id?.name || 'N/A'}</td>
                                    <td className="px-6 py-4">{c.lawyer_id?.name || 'Unassigned'}</td>
                                    {user?.role !== 'admin' && <td className="px-6 py-4">{c.petitioner}</td>}
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-card w-full max-w-2xl border border-border rounded-2xl shadow-2xl overflow-hidden scale-in-center transition-transform">
                        <div className="p-6 border-b border-border flex items-center justify-between">
                            <h2 className="text-xl font-bold">{editingCase ? 'Edit Case' : 'Create New Case'}</h2>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                {editingCase && (
                                    <div className="space-y-1">
                                        ...
                                        <div className="w-full bg-accent/20 border border-border rounded-lg px-4 py-2 text-sm font-mono text-primary select-none">
                            ...
                            <div className="flex justify-end gap-3 pt-4 border-t border-border">
                                ...
                                <button type="submit" className="px-6 py-2 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 text-sm font-bold shadow-lg">
